/* Adapted from Group_Contents_14.sas. The original LIBNAME/PROC IMPORT pointed
   at a classroom shared drive (~/STAT5740HW/...) and a CSV not included in the
   repo; here mylib is WORK and the same-shaped sample rows (matching the
   columns of the Kaggle youth-smoking-and-drug dataset the project analyzes)
   are supplied inline via DATALINES. The DATA step copy, PROC PRINT var list,
   PROC CONTENTS call, and titles are unchanged from the author's script. */
